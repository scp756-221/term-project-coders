...
def create_user():
    ...
    try:
        ...
        return json.dumps({"message": "error reading arguments"})
    url = db['name'] + '/' + db['endpoint'][1] # http://cmpt756db:30002/api/v1/datastore/write
...
def delete_user(user_id):
    ...
    if 'Authorization' not in headers:
        return Response(json.dumps({"error": "missing auth"}),
                        status=401,
                        mimetype='application/json')
    url = db['name'] + '/' + db['endpoint'][2] # http://cmpt756db:30002/api/v1/datastore/delete
...
def get_user(user_id):
    ...
    url = db['name'] + '/' + db['endpoint'][0] # # http://cmpt756db:30002/api/v1/datastore/read
